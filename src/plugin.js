    ...
    fetch('https://raw.githubusercontent.com/globe-coder/minitransat-tracker/master/boats_result.json')
        ...
        .then(result => {
            ...
            for (const boatName of Object.keys(result)) {
                ...
                // Add this code after adding the marker to the map
                marker.on('click', () => {
                    layer.setStyle({ weight: 4, opacity: 1 }); // Change style on click
                });

                // Add this code along with your existing mouseover and mouseout event listeners
                marker.on('mouseover', () => layer.setStyle({ weight: 4, opacity: 1 }));
                marker.on('mouseout', () => {
                    if (!marker._icon.classList.contains('leaflet-popup-target')) { // Check if it's not a popup click
